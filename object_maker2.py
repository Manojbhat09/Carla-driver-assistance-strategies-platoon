...
# ==============================================================================
DISTANCE = 300
# START_POSITION1 = carla.Transform(carla.Location(x=DISTANCE, y=45.5, z=2.0))
...
def game_loop(args):
    ...
    try:
        ...
        spawn_points = [carla.Transform(carla.Location(startposition.location.x+i*10,startposition.location.y,startposition.location.z)) for i in range(count)]
...
def start(btn):
    if DISTANCE:
        app.infoBox("Starting...", "Distance set, object will be spawned")
        app.stop()
...
def registerValues(btn):
    ...
    global DISTANCE
    if btn == "Set distance":
        value = app.getOptionBox("Options")
        print(value)
        DISTANCE = int(value)
        np.save("dist_save",int(value))
        app.removeLabel('f1')
...
def main():
    ...
    app.addMessage("info", "OBJECT MAKER")
    app.addFlashLabel("f1", "variables not initialized")
    app.addGrip(10,0)

    app.addOptionBox("Options", ["- Distances -", "100", "200",
                            "300", "400", "500","600","700","800", "900"])

    app.addButtons(["Set distance"], registerValues)
