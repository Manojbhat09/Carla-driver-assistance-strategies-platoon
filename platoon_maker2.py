...
def game_loop(args):
    ...
    try:
        ...
        spawn_points = [carla.Transform(carla.Location(startposition.location.x+i*40,startposition.location.y,startposition.location.z)) for i in range(count)]
        print("gotspawnpoints")
        idlist = []
        for i,spawn_point in enumerate(spawn_points):
            [vehicle , boolean] = try_spawn_random_vehicle_at(spawn_point,world,actor_list_n)
            if boolean:
                count -= 1
            if count <= 0:
                break
        ...
        print(idlist)
        time.sleep(5)
        actor_list = [x for x in world.get_actors().filter('vehicle.tesla.model3')]
        actor_list2 = []
        for x in actor_list:
            if x.id in idlist:
                actor_list2.append(x)
        ...
        while True:
            #client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=15)) for x in actor_list2])
            #print(sorted(actor_list)[0].get_velocity())
            #time.sleep(2)
            clock.tick_busy_loop(60)
            if sorted(actor_list)[0].get_velocity().x < 10:
                client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=15)) for x in actor_list])
            #clock.tick_busy_loop(60)
