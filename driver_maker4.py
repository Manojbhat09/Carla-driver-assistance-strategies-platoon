...
class World(object):
    def __init__(self, carla_world, hud, actor_filter,START_POSITIONk, writer):
    ...
    def tick(self, clock):
        self.hud.tick(self, clock, self.writer)
...
class HUD(object):
    ...
    def tick(self, world, clock, writer):
        ...
        self.write_count+=1
        write_count = self.write_count
        if self.set_vehicles == 0:
            vehicles = world.world.get_actors().filter('vehicle.*')
            self.vehicles = sorted(vehicles)
            self.vehicle_list = sorted([x.id for x in vehicles])
            self.set_vehicles = 1

        vel_mag = (3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2))
        acc_mag = (3.6 * math.sqrt(a.x**2 + a.y**2 + a.z**2))
        ...
        distance = lambda l: math.sqrt((l.x - t.location.x)**2 + (l.y - t.location.y)**2 + (l.z - t.location.z)**2)
        #print(self.vehicles)
        vehicles_distance = [distance(x.get_location()) for x in self.vehicles if x.id != world.player.id] 
        ...
        if len(self.print_list) == 0:
            self.print_list = [listing+self.vehicle_list+vehicles_distance]
        else:
            self.print_list.append(listing+self.vehicle_list+vehicles_distance)
            #print("appended")
        # if write_start == 0:
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        #1.6,1.7
            ...
            carla.Transform(carla.Location(x=0.5, z=3.8), carla.Rotation(pitch=-15))]
...
def game_loop(args):
    ...
    pygame.font.init()
    world = None
    dest = np.load("dist_save.npy")
    vel = np.load("vel_save.npy")
    sldist = np.load("slot_dist_save.npy")
    file = open('../saved_data/saved_egocar_'+args.dest+"_"+str(sldist)+"_"+str(vel)+"_"+str(dest)+'.csv',"wb")
    ...
    try:
        ...
        world = World(client.get_world(), hud, args.filter,startposition,writer)
        ...
        while True:
            ...
            spectator_transform = carla.Transform(carla.Location(transform.location.x+150,transform.location.y,transform.location.z+180))
        ...
        world_client = client.get_world()
        actor_list = [x for x in world_client.get_actors().filter('vehicle.*')] + [x for x in world_client.get_actors().filter('sensor.*')] 
...
app = gui("Demo GUI", "500x500",handleArgs=False)
...
def main():
    ...
    app.addMessage("info", "DRIVER MAKER")
