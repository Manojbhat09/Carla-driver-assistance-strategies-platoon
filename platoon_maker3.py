...
START_POSITION1 = carla.Transform(carla.Location(x=0.0, y=49, z=2.0))
...
def try_spawn_random_vehicle_at(transform,world,actor_list):
    blueprint = world.get_blueprint_library().find('vehicle.toyota.prius')
    print("reachedBP")
    if blueprint.has_attribute('color'):
        color = random.choice(blueprint.get_attribute('color').recommended_values)
        blueprint.set_attribute('color', color)
    #blueprint.set_attribute("sticky_control", "True")
...
def game_loop(args):
    ...
    try:
        ...
        #spectator_transform = carla.Transform(carla.Location(startposition.location.x+50,startposition.location.y,startposition.location.z+100))
        #spectator_transform.rotation.pitch = -90.0
        #spectator.set_transform(spectator_transform)
        count = args.number_of_vehicles
        print(dir(world.get_map().get_spawn_points()[0]))
        spawn_points = [carla.Transform(carla.Location(startposition.location.x+i*SLOT_LENGTH,startposition.location.y,startposition.location.z)) for i in range(count)]
        
        print("gotspawnpoints")
        for spawn_point in spawn_points:
            if try_spawn_random_vehicle_at(spawn_point,world,actor_list):
                count -= 1
            if count <= 0:
                break

        # print(world.get_blueprint_library().filter('vehicle'))
        # exit()
        time.sleep(5)
        actor_list = [x for x in world.get_actors().filter('vehicle.toyota.prius')]
        actor_list2 = actor_list+[x for x in world.get_actors().filter('vehicle.tesla.model3')]
        print("got actors")
        # for vehicle in actor_list:
        #     vehicle.set_velocity(carla.Vector3D(x =15, y = 0, z=0))
        #actors = [x for x in world.get_actors() if x.id != world.player.id]
        client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=VELOCITY)) for x in actor_list2])
        ...
        while True:
            if sorted(actor_list)[0].get_velocity().x < VELOCITY-5:
                client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=VELOCITY)) for x in actor_list])
            #print(sorted(actor_list)[0].get_velocity())
...
def start(btn):
    if SLOT_LENGTH:
        app.infoBox("Starting...", "Variables set, object will be spawned")
        app.stop()

def usePrev(btn):
    global SLOT_LENGTH
    global VELOCITY
    if btn=="Use Previous settings":
        dest = np.load("slot_dist_save.npy")
        SLOT_LENGTH = int(dest)
        vel = np.load("vel_save.npy")
        VELOCITY = int(vel)
        app.removeLabel('f1')
        app.removeLabel('f2')
        app.stop()
...
def registerValues(btn):
    ...
    global SLOT_LENGTH
    global VELOCITY
    if btn == "Set slot_length":
        value = app.getOptionBox("Options_slot")
        print(value)
        SLOT_LENGTH = int(value)
        np.save("slot_dist_save",int(value))
        app.removeLabel('f1')

    if btn == "set velocity":
        value = app.getOptionBox("Options_vel")
        print(value)
        VELOCITY = int(value)
        np.save("vel_save",int(value))
        app.removeLabel('f2')

app = gui("Demo GUI", "500x500")
...
def main():
    ...
    app.setFg("Black")
    app.setFont(20)
    app.addLabel("title", "Platoon simulator")
    app.addMessage("info", "PLATOON MAKER")
    app.addFlashLabel("f1", "slot_length not initialized")
    app.addFlashLabel("f2", "velocity not initialized")
    app.addGrip(10,0)

    app.addOptionBox("Options_slot", ["- SLOT_LENGTH -", "10", "20",
                            "30", "40", "50","60","70","80", "90"])
    app.addOptionBox("Options_vel", ["- VELOCITY -", "5", "10",
                        "15", "20", "25","30","35","40", "50", "60","70","80","90","100","120"])

    app.addButtons(["Set slot_length", "set velocity"], registerValues)

    app.addButton("Start", start)

    app.addButton("Use Previous settings", usePrev)
