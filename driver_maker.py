...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if not self._show_info:
            ...
            distance = lambda l: math.sqrt((l.x - t.location.x)**2 + (l.y - t.location.y)**2 + (l.z - t.location.z)**2)
            vehicle_list = [x.id for x in vehicles]
            vehicles_distance = [distance(x.get_location()) for x in vehicles if x.id != world.player.id] 
            
            listing = [player_id, frame_number, time, self.simulation_time,x_,y_,z_,vel_mag,angle, heading, throttle, brake, steer, impulse]
            ...
            if self.write_count>200:
                with open('saved_egocar.csv',"wb") as f:
                    writer = csv.writer(f)
...
##
    ...
    def set_sensor(self, index, notify=True):
        ...
        if needs_respawn:
            ...
            self.m
            ini_sensor2.listen(lambda image: CameraManager._parse_image(weak_self, image,self.mini_index2))
