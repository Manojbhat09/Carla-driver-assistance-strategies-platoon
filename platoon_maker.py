...
def game_loop(args):
    ...
    try:
        ...
        client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=VELOCITY-5)) for x in actor_list2])
        ...
        while True:
            if sorted(actor_list)[0].get_velocity().x < VELOCITY-15:
                client.apply_batch([carla.command.ApplyVelocity(x.id, carla.Vector3D(x=VELOCITY-5)) for x in actor_list])
...
def main():
    ...
    app.addOptionBox("Options_vel", ["- VELOCITY -", "10", "20",
